...
async def ensure_tables():
    ...
    try:
        ...
        # ===== REPORTS TABLE (with geom column) =====
        await conn.execute("""
...
async def get_admin_stats(days: int = 30):
    ...
    try:
        ...
        top_reporters = await conn.fetch(f"""
            ...
            FROM reports WHERE is_current = 1 {date_filter}
        ...
        daily_trend = await conn.fetch(f"""
            ...
            FROM reports WHERE is_current = 1 AND timestamp::timestamp >= (NOW() - INTERVAL '{days_limit} days')
        ...
        by_infrastructure = await conn.fetch(f"""
            ...
            WHERE is_current = 1 AND infrastructure_type IS NOT NULL AND infrastructure_type != '' {date_filter}
        ...
        by_crisis = await conn.fetch(f"""
            ...
            WHERE is_current = 1 AND crisis_nature IS NOT NULL AND crisis_nature != '' {date_filter}
        ...
        for r in rows:
            if r[1] and r[2]:
                features.append({
                    ...
                    "properties": {"damage_level": r[0], "infrastructure_type": r[3], "crisis_nature": r[4], "building_name": r[5], "timestamp": r[6]}
                })
        return {"type": "FeatureCollection", "features": features}
    finally:
        await conn.close()
