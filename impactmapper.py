...
async def get_admin_stats(days: int = 30):
    ...
    try:
        ...
        # total distinct users (from reports)
        ...
        # daily trend (last 7 days or for the filtered period)
        if days > 0:
            days_limit = min(days, 30)  # cap at 30 for performance
        else:
            days_limit = 30
        ...
        # users by role (from users table)
        ...
        # avg response time? we'll skip for now, we can compute from timestamp differences if needed
...
async def admin_stats(days: int = 7, current_user: dict = Depends(require_admin)):
    """Admin stats for analytics dashboard. days=0 means all time."""
...
# ============================================
# UNIFIED DASHBOARD HTML – with scrollable sidebar, working charts, analytics filter
# ============================================
...
<div id="analyticsTab" class="tab-content">
    <div style="padding:12px 20px; overflow-y:auto; height:100%;">
        <div class="analytics-filter">
            <label style="color:#aaa; font-size:0.8rem;"><i class="fas fa-calendar-alt"></i> Date Range:</label>
    ...
    try {
        ...
        trendChart = new Chart(document.getElementById('trendChart'), {
            type: 'line', data: { 
                ...
                datasets: [{ label:'Reports', data:data.daily_trend.map(d=>d.count), borderColor:'#2ecc71', fill:true, backgroundColor:'rgba(46,204,113,0.1)', tension:0.4 }]
            },
        ...
        damageChart = new Chart(document.getElementById('damageChart'), {
            type: 'doughnut', data: { 
                labels: data.by_damage.map(d=>d.level), 
                datasets: [{ data:data.by_damage.map(d=>d.count), backgroundColor:['#e74c3c','#f39c12','#2ecc71'] }]
            },
        ...
        infraChart = new Chart(document.getElementById('infraChart'), {
            type: 'bar', data: { 
                ...
                datasets: [{ label:'Reports', data:data.by_infrastructure.map(d=>d.count), backgroundColor:'#2ecc71', borderRadius:8 }]
            },
            options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { labels: { color:'#e0e0e0' } } } }
        });
        ...
        crisisChart = new Chart(document.getElementById('crisisChart'), {
            type: 'bar', data: { 
                ...
                datasets: [{ label:'Reports', data:data.by_crisis.map(d=>d.count), backgroundColor:'#3498db', borderRadius:8 }]
            },
            options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { labels: { color:'#e0e0e0' } } } }
        });
...
async function loadCurrentUser() {
    try {
        ...
        if(user.role === 'admin') { 
            ...
            isAdmin=true; 
            loadAdminStats(); // load analytics if admin
        } else { 
