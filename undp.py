...
app = FastAPI(title="UNDP ImpactMapper", version="26.0.0")
security = HTTPBasic()
...
for username, pwd_hash, role, avatar, color, points, verified, badge, phone in default_users:
    cursor.execute("SELECT * FROM users WHERE username = ?", (username,))
    if not cursor.fetchone():
        cursor.execute("INSERT INTO users (username, password_hash, role, avatar, color, points, verified_reports, badge_level, created_at, phone_number) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                       (username, pwd_hash, role, avatar, color, points, verified, badge, datetime.now().isoformat(), phone))
...
def save_report(report_uuid: str, building_id: str, building_osm_id: str, building_name: str, building_address: str,
...
def get_leaderboard(limit: int = 15):
    conn = sqlite3.connect("reports.db")
    cursor = conn.cursor()
    cursor.execute("SELECT username, points, verified_reports, badge_level, avatar, color FROM users ORDER BY points DESC LIMIT ?", (limit,))
    leaders = [{"username": r[0], "points": r[1], "verified_reports": r[2], "badge": r[3], "avatar": r[4], "color": r[5]} for r in cursor.fetchall()]
    conn.close()
    return leaders
...
def verify_user(credentials: HTTPBasicCredentials = Depends(security)):
    conn = sqlite3.connect("reports.db")
    cursor = conn.cursor()
    cursor.execute("SELECT password_hash, role, avatar, color, points, badge_level FROM users WHERE username = ?", (credentials.username,))
    ...
    if password_hash != user[0]:
        raise HTTPException(status_code=401, detail="Invalid credentials")
    return {"username": credentials.username, "role": user[1], "avatar": user[2], "color": user[3], "points": user[4], "badge": user[5]}
...
async def admin_stats(current_user: dict = Depends(require_admin)):
    ...
    cursor.execute("SELECT damage_level, COUNT(*) FROM reports WHERE is_current = 1 GROUP BY damage_level")
    by_damage = [{"level": r[0] or "Unknown", "count": r[1]} for r in cursor.fetchall()]
    
    cursor.execute("SELECT DATE(timestamp) as day, COUNT(*) FROM reports WHERE DATE(timestamp) >= DATE('now', '-30 days') GROUP BY DATE(timestamp) ORDER BY day")
    daily_trend = [{"date": r[0], "count": r[1]} for r in cursor.fetchall()]
    
    cursor.execute("SELECT infrastructure_type, COUNT(*) FROM reports WHERE is_current = 1 GROUP BY infrastructure_type")
    by_infrastructure = [{"type": r[0] or "Unknown", "count": r[1]} for r in cursor.fetchall()]
    
    cursor.execute("SELECT crisis_nature, COUNT(*) FROM reports WHERE is_current = 1 GROUP BY crisis_nature")
    by_crisis = [{"crisis": r[0] or "Unknown", "count": r[1]} for r in cursor.fetchall()]
    
    cursor.execute("SELECT COUNT(*) FROM users")
    total_users = cursor.fetchone()[0]
    
    cursor.execute("SELECT role, COUNT(*) FROM users GROUP BY role")
    users_by_role = [{"role": r[0], "count": r[1]} for r in cursor.fetchall()]
    
    cursor.execute("SELECT username, COUNT(*) as report_count FROM reports WHERE is_current = 1 GROUP BY username ORDER BY report_count DESC LIMIT 10")
    top_reporters = [{"username": r[0] or "Anonymous", "reports": r[1]} for r in cursor.fetchall()]
    
    cursor.execute("SELECT COUNT(*) FROM reports WHERE is_current = 1")
...
async def create_report(
    ...
    conn = sqlite3.connect("reports.db")
    cursor = conn.cursor()
    cursor.execute("UPDATE users SET points = points + 10, verified_reports = verified_reports + 1 WHERE username = ?", (current_user['username'],))
    conn.commit()
    conn.close()
...
async def sync_offline_reports(reports_data: List[Dict], current_user: dict = Depends(require_reporter)):
    ...
    for report in reports_data:
        try:
            conn = sqlite3.connect("reports.db")
            cursor = conn.cursor()
            cursor.execute("SELECT report_uuid FROM reports WHERE report_uuid = ?", (report.get('report_uuid'),))
            if not cursor.fetchone():
                cursor.execute("""
                ...
                conn.commit()
                synced_count += 1
                cursor.execute("UPDATE users SET points = points + 10 WHERE username = ?", (current_user['username'],))
                conn.commit()
            conn.close()
        except Exception as e:
...
async def get_reports(limit: int = 200, current_user: dict = Depends(verify_user)):
    ...
    cursor.execute("""
        ...
        FROM reports WHERE is_current = 1 ORDER BY timestamp DESC LIMIT ?
    """, (limit,))
...
async def get_geojson(current_user: dict = Depends(require_admin)):
    conn = sqlite3.connect("reports.db")
    cursor = conn.cursor()
    cursor.execute("SELECT damage_level, lat, lng, infrastructure_type, crisis_nature, building_name, timestamp FROM reports WHERE lat != 0 AND is_current = 1")
    rows = cursor.fetchall()
    conn.close()
    features = []
    for r in rows:
...
async def export_csv(current_user: dict = Depends(require_admin)):
    conn = sqlite3.connect("reports.db")
    cursor = conn.cursor()
    cursor.execute("SELECT damage_level, lat, lng, building_name, building_address, infrastructure_type, crisis_nature, debris, notes, timestamp, username FROM reports WHERE is_current = 1 ORDER BY timestamp DESC")
    rows = cursor.fetchall()
    conn.close()
...
async def get_stats():
    ...
    cursor.execute("SELECT COUNT(*) FROM reports WHERE DATE(timestamp) = ? AND is_current = 1", (today,))
    today_count = cursor.fetchone()[0]
    cursor.execute("SELECT COUNT(*) FROM reports WHERE synced = 0")
...
async def websocket_endpoint(websocket: WebSocket, username: str):
    conn = sqlite3.connect("reports.db")
    cursor = conn.cursor()
    cursor.execute("SELECT role, avatar, color, points, badge_level FROM users WHERE username = ?", (username,))
    user = cursor.fetchone()
    conn.close()
    if not user:
...
function updateCommandCenterCharts() {
    // Update Pie Chart (Damage Distribution)
    ...
    // Update Bar Chart (Infrastructure Distribution)
    const infraCounts = {};
    for (let r of reports) {
    ...
    // Update Line Chart (Daily Trend - Last 7 days)
    const last7Days = [];
    const dailyCounts = {};
    for (let r of reports) {
...
async function setLanguage(lang) {
    ...
    try {
        ...
        document.getElementById('reportTitle').innerText = translations.report_damage;
        document.getElementById('recentTitle').innerText = translations.recent_reports;
        document.getElementById('exportTitle').innerText = translations.export_data + (isAdmin ? ' (Admin Only)' : '');
        document.getElementById('gpsLabel').innerText = translations.gps_location;
        document.getElementById('submitLabel').innerText = translations.submit;
        document.getElementById('csvLabel').innerText = translations.export_csv;
        document.getElementById('geojsonLabel').innerText = translations.export_geojson;
        document.getElementById('clickHint').innerHTML = translations.click_building;
        document.getElementById('chatInput').placeholder = translations.type_message;
        document.getElementById('smsTitle').innerText = translations.sms_report;
        document.getElementById('smsSendLabel').innerText = translations.sms_send;
        document.getElementById('smsText').placeholder = translations.sms_placeholder;
        document.getElementById('tabCommandBtn').innerHTML = `<i class="fas fa-map-marked-alt"></i> ${translations.command_center || 'Command Center'}`;
        document.getElementById('tabAnalyticsBtn').innerHTML = `<i class="fas fa-chart-line"></i> ${translations.analytics || 'Analytics Dashboard'}`;
        const damageOpts = document.querySelectorAll('#damageLevel option');
        if (damageOpts.length >= 3) {
            damageOpts[0].innerText = translations.minimal;
            damageOpts[1].innerText = translations.partial;
            damageOpts[2].innerText = translations.complete;
        }
...
function initMap() {
    ...
    map.on('click', async function(e) {
        ...
        try {
            const response = await fetch(`/api/building/${lat}/${lng}`);
            const building = await response.json();
            if (building && building.osm_id) {
                selectedBuilding = building;
                document.getElementById('lat').value = lat.toFixed(6);
                document.getElementById('lng').value = lng.toFixed(6);
                document.getElementById('buildingName').value = building.name || building.building_type || '';
                document.getElementById('selectedBuildingInfo').style.display = 'block';
                document.getElementById('selectedBuildingInfo').innerHTML = `🏢 Building: ${building.name || building.building_type}<br>📍 ${building.address || ''}`;
                if (currentMarker) map.removeLayer(currentMarker);
                currentMarker = L.marker([lat, lng]).bindPopup('Selected building').addTo(map);
...
function shareLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(pos) {
            ...
            if (currentMarker) map.removeLayer(currentMarker);
            currentMarker = L.marker([pos.coords.latitude, pos.coords.longitude]).bindPopup('Your GPS location').addTo(map);
            if (ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: 'location', lat: pos.coords.latitude, lng: pos.coords.longitude }));
            showToast('📍 GPS location shared!');
        });
...
async function sendSMSReport() {
    ...
    if (!smsText) { showToast('Please enter SMS in format: DAMAGE LAT LNG', 'error'); return; }
    document.getElementById('smsStatus').innerHTML = '<span style="color:#f39c12;">📱 Sending SMS report...</span>';
    try {
        const formData = new FormData();
        formData.append('sms_text', smsText);
        formData.append('sms_number', smsNumber);
        const response = await fetch('/api/sms_report', { method: 'POST', body: formData });
        const result = await response.json();
        if (result.status === 'success') {
            document.getElementById('smsStatus').innerHTML = '<span style="color:#2ecc71;">✅ SMS report received! Location added to map.</span>';
            document.getElementById('smsText').value = '';
            loadReports();
            showToast(`SMS report added at location: ${result.lat}, ${result.lng}`);
        } else {
            document.getElementById('smsStatus').innerHTML = '<span style="color:#e74c3c;">❌ Invalid SMS format. Use: DAMAGE LAT LNG</span>';
        }
    } catch(error) {
        document.getElementById('smsStatus').innerHTML = '<span style="color:#e74c3c;">❌ Failed to send SMS report</span>';
    }
}
...
async function submitReport() {
    ...
    formData.append('building_address', selectedBuilding?.address || '');
    formData.append('building_osm_id', selectedBuilding?.osm_id || '');
    ...
    if (lat && lng) { formData.append('lat', parseFloat(lat)); formData.append('lng', parseFloat(lng)); }
    formData.append('text_location', document.getElementById('textLocation').value);
    formData.append('notes', document.getElementById('notes').value);
    const photo = document.getElementById('photo').files[0];
    if (photo) formData.append('photo', photo);
    document.getElementById('submitStatus').innerHTML = '<span style="color:#f39c12;">⏳ Submitting...</span>';
    try {
        const response = await fetch('/api/report', { method: 'POST', body: formData });
        const result = await response.json();
        if (result.status === 'success') {
            document.getElementById('submitStatus').innerHTML = '<span style="color:#2ecc71;">✅ Report submitted! +10 points</span>';
            ...
            loadReports(); loadStats(); loadUserStats(); loadLeaderboard();
            updateKPIs();
            updateCommandCenterCharts();
            if (result.lat && result.lng) map.setView([result.lat, result.lng], 18);
        }
    } catch(error) {
        offlineQueue.push({ damage_level: document.getElementById('damageLevel').value, infrastructure_type: document.getElementById('infrastructureType').value, building_name: document.getElementById('buildingName').value, crisis_nature: document.getElementById('crisisNature').value, debris: document.getElementById('debris').value, lat: parseFloat(lat) || null, lng: parseFloat(lng) || null, text_location: document.getElementById('textLocation').value, notes: document.getElementById('notes').value, timestamp: new Date().toISOString() });
        saveOfflineQueue();
        document.getElementById('submitStatus').innerHTML = '<span style="color:#f39c12;">⚠️ Saved offline - will sync when online</span>';
        showToast('Report saved offline!');
        loadReports();
        updateKPIs();
        updateCommandCenterCharts();
    }
...
async function syncOfflineReports() {
    if (!navigator.onLine) { showToast('Cannot sync: You are offline', 'error'); return; }
    if (offlineQueue.length === 0) { showToast('No pending reports to sync'); return; }
    showToast(`Syncing ${offlineQueue.length} reports...`);
    let synced = [];
    for (let report of offlineQueue) {
        try {
            ...
            formData.append('building_name', report.building_name || '');
            formData.append('crisis_nature', report.crisis_nature);
            formData.append('debris', report.debris);
            if (report.lat && report.lng) { formData.append('lat', report.lat); formData.append('lng', report.lng); }
            formData.append('text_location', report.text_location || '');
            formData.append('notes', report.notes || '');
            const response = await fetch('/api/report', { method: 'POST', body: formData });
            if (response.ok) synced.push(report);
        } catch(e) {}
    }
    if (synced.length > 0) {
        offlineQueue = offlineQueue.filter(r => !synced.includes(r));
        saveOfflineQueue();
        showToast(`✅ Synced ${synced.length} reports!`);
        loadReports(); loadStats(); loadUserStats(); loadLeaderboard();
        updateKPIs();
        updateCommandCenterCharts();
    } else { showToast('No reports could be synced'); }
}

async function forceSync() { showToast('Force syncing...'); await syncOfflineReports(); }
...
function updateKPIs() {
    document.getElementById('activeCases').innerHTML = reports.length;
    document.getElementById('activeCases').className = reports.length > 20 ? 'kpi-value critical' : (reports.length > 10 ? 'kpi-value warning' : 'kpi-value stable');
    const capacity = Math.min(100, (reports.length / 50) * 100);
    document.getElementById('capacityBar').style.width = capacity + '%';
    let critical = 0, high = 0;
    for (let r of reports) {
        if (r.damage_level === 'complete') critical++;
        else if (r.damage_level === 'partial') high++;
    }
    document.getElementById('criticalCount').innerHTML = critical;
    document.getElementById('highCount').innerHTML = high;
    const deployed = Math.min(12, Math.ceil(reports.length / 10));
    const standby = 12 - deployed;
    document.getElementById('resourcesDeployed').innerHTML = deployed;
    document.getElementById('deployedCount').innerHTML = deployed;
    document.getElementById('standbyCount').innerHTML = standby;
    document.getElementById('resourceBar').style.width = (deployed / 12 * 100) + '%';
    const activeVol = Math.min(350, Math.floor(reports.length * 1.5));
    const standbyVol = Math.max(0, 100 - activeVol);
    const offlineVol = 350 - activeVol - standbyVol;
    document.getElementById('totalVolunteers').innerHTML = 350;
    document.getElementById('activeVolunteersCount').innerHTML = activeVol;
    document.getElementById('standbyVolunteers').innerHTML = standbyVol;
    document.getElementById('offlineVolunteers').innerHTML = offlineVol;
    const pending = offlineQueue.length;
    document.getElementById('pendingTasks').innerHTML = pending;
    document.getElementById('urgentTasks').innerHTML = Math.min(5, pending);
    if (pending > 10) { document.getElementById('pendingTasks').className = 'kpi-value critical';
    } else if (pending > 5) { document.getElementById('pendingTasks').className = 'kpi-value warning';
    } else { document.getElementById('pendingTasks').className = 'kpi-value stable'; }
}

function updateMapMarkers() {
    markers.forEach(m => map.removeLayer(m));
    markers = [];
    const colors = { minimal: '#2ecc71', partial: '#f39c12', complete: '#e74c3c' };
    for (let r of reports) {
        if (r.lat && r.lng && r.lat !== 0) {
            const marker = L.circleMarker([r.lat, r.lng], { radius: 8, fillColor: colors[r.damage_level] || '#2ecc71', color: 'white', weight: 1.5, fillOpacity: 0.8 }).bindPopup(`<b>🚨 ${r.damage_level.toUpperCase()}</b><br>📍 ${r.infrastructure_type}<br>🏢 ${r.building_name || 'Building'}<br>👤 By: ${r.username || 'SMS User'}`).addTo(map);
            markers.push(marker);
        }
    }
}
...
function updateReportsList() {
    ...
    if (reports.length === 0) { container.innerHTML = '<div style="text-align:center; padding:15px;">No reports yet. Click a building!</div>'; return; }
    let html = '';
    for (let r of reports.slice(0, 15)) {
        const badgeClass = r.damage_level === 'minimal' ? 'badge-minimal' : (r.damage_level === 'partial' ? 'badge-partial' : 'badge-complete');
        const severityClass = r.damage_level === 'complete' ? 'severity-critical' : (r.damage_level === 'partial' ? 'severity-high' : 'severity-normal');
        html += `<div class="report-item ${severityClass}" onclick="if(${r.lat}) map.setView([${r.lat}, ${r.lng}], 18)">
            <strong>${(r.damage_level || '').toUpperCase()}</strong>
            <span class="damage-badge ${badgeClass}">${r.damage_level}</span>
            ${r.is_offline ? '<span class="pending-badge">Pending</span>' : ''}<br>
            🏢 ${r.building_name || 'Building'}<br>
            📍 ${r.infrastructure_type}<br>
            👤 ${r.username || 'SMS'}
        </div>`;
    }
...
function updateConnectionStatus(isOnline) {
    ...
    if (isOnline) { statusDiv.className = 'status-badge status-online'; statusDiv.innerHTML = '<i class="fas fa-circle" style="font-size: 5px;"></i> Online';
    } else { statusDiv.className = 'status-badge status-offline'; statusDiv.innerHTML = '<i class="fas fa-circle" style="font-size: 5px;"></i> Offline'; }
}
...
async function loadCurrentUser() {
    try {
        ...
        document.getElementById('userPoints').innerHTML = currentUser.points;
        document.getElementById('userBadge').innerHTML = currentUser.badge;
        ...
        if (isAdmin) {
            ...
            document.getElementById('tabAnalyticsBtn').style.display = 'none';
        ...
        setInterval(() => {
            if (navigator.geolocation && ws && ws.readyState === WebSocket.OPEN) {
                navigator.geolocation.getCurrentPosition(function(pos) { ws.send(JSON.stringify({ type: 'location', lat: pos.coords.latitude, lng: pos.coords.longitude })); }, () => {});
            }
        }, 30000);
    } catch(e) {}
}
...
function connectWebSocket() {
    const wsUrl = `ws://${window.location.host}/ws/${currentUser.username}`;
    ...
    ws.onmessage = (event) => {
        ...
        else if (data.type === 'new_report') { loadReports(); showToast(`📢 New report from ${data.data.username}!`); updateKPIs(); updateCommandCenterCharts(); }
    };
    ws.onclose = () => { setTimeout(connectWebSocket, 3000); };
}

function updatePresence(users, count) {
    document.getElementById('presenceCount').innerHTML = count;
...
function updateLiveContributors(contributors) {
    contributorMarkers.forEach(m => map.removeLayer(m));
    contributorMarkers = [];
    for (let c of contributors) {
        if (c.lat && c.lng && c.username !== currentUser.username) {
            const marker = L.marker([c.lat, c.lng], { icon: L.divIcon({ html: `<div style="background:${c.color};border-radius:50%;width:24px;height:24px;display:flex;align-items:center;justify-content:center;border:2px solid white;font-size:12px;">${c.avatar}</div>`, iconSize: [24,24] }) }).bindPopup(`${c.username} (${c.role})`).addTo(map);
...
function addChatMessage(msg) {
    ...
    div.innerHTML = `<div style="font-size:8px;color:${msg.color}">${msg.username}</div><div>${msg.message}</div>`;
    container.appendChild(div);
    container.scrollTop = container.scrollHeight;
}

async function loadLeaderboard() { try { const res = await fetch('/api/leaderboard'); const leaders = await res.json(); document.getElementById('leaderboardList').innerHTML = leaders.map((l, idx) => `<div class="leaderboard-item"><span class="rank">${idx+1}</span><span>${l.avatar}</span><span style="flex:1">${l.username}</span><span style="color:#2ecc71">${l.points}</span></div>`).join(''); } catch(e) {} }
async function loadStats() { try { const res = await fetch('/api/stats'); const stats = await res.json(); } catch(e) {} }
async function loadUserStats() { try { const res = await fetch(`/api/user_stats/${deviceId}`); const stats = await res.json(); document.getElementById('userReports').innerHTML = stats.reports; } catch(e) {} }
function exportCSV() { window.open('/api/reports/csv', '_blank'); }
async function exportGeoJSON() { const res = await fetch('/api/reports/geojson'); const data = await res.json(); const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([JSON.stringify(data)], { type: 'application/json' })); a.download = `reports_${new Date().toISOString()}.geojson`; a.click(); }
function showToast(msg, type = 'success') { const t=document.createElement('div'); t.style.cssText=`position:fixed;bottom:100px;left:50%;transform:translateX(-50%);background:${type === 'error' ? '#e74c3c' : '#2ecc71'};color:white;padding:8px 16px;border-radius:20px;z-index:2000;font-weight:500;font-size:11px;`; t.innerText=msg; document.body.appendChild(t); setTimeout(()=>t.remove(),2500); }
function togglePresence() { const l=document.getElementById('presenceList'); l.style.display=l.style.display==='none'?'block':'none'; }
function toggleChat() { const m=document.getElementById('chatMessages'); const i=document.querySelector('.chat-input-area'); m.style.display=m.style.display==='none'?'flex':'none'; i.style.display=i.style.display==='none'?'flex':'none'; }
function toggleLeaderboard() { const l=document.getElementById('leaderboardList'); l.style.display=l.style.display==='none'?'block':'none'; }
...
if __name__ == "__main__":
    print("=" * 80)
    print("🏆 UNDP ImpactMapper - ANALYTICS COMMAND CENTER")
    print("=" * 80)
    print("✅ Added 3 Charts in Command Center: Pie Chart, Bar Chart, Line Chart")
    print("✅ Chart area: Whitish with 80% transparency")
    print("✅ X and Y axes: Black text with Arial Black font")
    print("✅ Report Damage Panel: Birds Eye Grey with 90% transparency")
    print("✅ Chat Panel: Whitish with 90% transparency")
    print("✅ KPI Boxes: Green blinking shadow effect on hover")
    print("✅ Header: Bottle Green with centered title")
    print("✅ Language, Status, Sync, Role, Logout: In-line format")
    print("✅ Model Name Centered in Header")
    print("✅ Unified Dashboard with Tabbed Interface")
    print("✅ Command Center with Map, Reports, Chat, Leaderboard + 3 Charts")
    print("✅ Analytics Dashboard with Charts and Statistics")
    print("✅ 6 Languages Support")
    print("✅ Offline-First with Auto-Sync")
    print("✅ SMS Reporting + Building Footprints")
    print("=" * 80)
    print("🌐 OPEN: http://localhost:8080")
    print("=" * 80)
    print("🔑 Demo Accounts:")
    print("   👑 admin / admin123 (Full access - Command Center + Analytics tabs)")
    print("   📸 reporter / report123 (Submit reports - Command Center only)")
    print("   👁️ viewer / view123 (View only - Command Center only)")
    print("=" * 80)
